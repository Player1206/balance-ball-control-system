...
/*发送部分*/
uint8_t NRF24L01_TxAddress[5] = {0x11, 0x22, 0x11, 0x22, 0x55};		//发送地址，固定5字节
#define NRF24L01_TX_PACKET_WIDTH		4							//发送数据包宽度，范围：1~32字节
uint8_t NRF24L01_TxPacket[NRF24L01_TX_PACKET_WIDTH];				//发送数据包

/*接收部分*/
uint8_t NRF24L01_RxAddress[5] = {0x11, 0x22, 0x11, 0x22, 0x55};		//接收通道0地址，固定5字节
...
void NRF24L01_GPIO_Init(void)
{
	...
	/* ===================== 配置输出引脚 ===================== */
	// 1. 配置 GPIOA：PA4(CE)、PA5(CSN) → 推挽输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   // 推挽输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5; // 严格对应CE、CSN引脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  // 高速输出
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	
	// 2. 配置 GPIOC：PC4(SCK)、PC5(MOSI) → 推挽输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   // 推挽输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5; // 严格对应SCK、MOSI引脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  // 高速输出
	GPIO_Init(GPIOC, &GPIO_InitStructure);

	/* ===================== 配置输入引脚 ===================== */
	// 3. 配置 GPIOB：PB0(MISO) → 上拉输入（输入模式无需配置速度）
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;      // 上拉输入（NRF24L01标准配置）
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;           // 严格对应MISO引脚
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	
	/* ===================== 设置默认电平 ===================== */
	NRF24L01_W_CE(0);		// CE默认为低电平，退出收发模式
	NRF24L01_W_CSN(1);		// CSN默认为高电平，不选中模块
	NRF24L01_W_SCK(0);		// SCK默认为低电平，匹配SPI模式0
	NRF24L01_W_MOSI(0);		// MOSI默认电平，无强制要求
}
