...
#include "step.h"     // Step_Parse: 解析步进电机回传位置

float Ax, Ay, Az;    // 加速度
float Wx, Wy, Wz;    // 角速度
float Roll, Pitch, Yaw;  // 角度
volatile uint32_t JY61P_RxCount = 0;       // JY61P 串口收到的原始字节数
volatile uint32_t JY61P_FrameCount = 0;    // JY61P 校验通过的有效帧数
volatile uint32_t JY61P_CheckError = 0;    // JY61P 校验失败次数
volatile uint32_t JY61P_SyncDrop = 0;      // JY61P 帧头/类型不同步丢弃次数
volatile uint8_t  JY61P_LastType = 0;      // 最近一次有效帧类型: 0x51加速度 0x52角速度 0x53角度
...
void JY61P_Decode(uint8_t Data)
{
    ...
    /* JY61P 标准帧固定 11 字节:
     *   0x55 + 类型(0x51/0x52/0x53) + 8字节数据 + 校验和
     *
     * 原来的写法是“收到任意11字节就校验”，如果中间丢了1个字节，
     * 后面会持续错位。这里改成按 0x55 帧头同步，数据更适合拿来做前馈。
     */
    if (len == 0)
    {
        if (Data != 0x55)
    ...
    if (len == 1)
    {
        ...
        {
            JY61P_SyncDrop++;
            len = (Data == 0x55) ? 1 : 0;  // 如果这个字节刚好是新帧头，直接作为新帧开始
            RX_Data[0] = 0x55;
            return;
        }
    }
...
// UART5 接收 —— 步进电机回传位置(SYS_CPOS)
void UART5_IRQHandler(void)
{
    if (USART_GetITStatus(UART5, USART_IT_RXNE) == SET)
    {
        uint8_t Data = (UART5->DR & 0xFF);
        Step_Parse(Data);
    }
